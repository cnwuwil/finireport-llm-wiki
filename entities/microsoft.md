---
title: Microsoft Corporation（MSFT）
type: entity
domain: invest
tags: [微软, Microsoft, MSFT, Azure, Copilot, M365, 云计算, GenAI, AI-基础设施]
created: 2026-04-21
updated: 2026-04-24
---

# Microsoft Corporation（MSFT）

**股票代码**：MSFT.O（纳斯达克）
**行业**：软件与云服务 / 人工智能基础设施
**市值**：$3.15 万亿（2026-04-20）
**总部**：美国华盛顿州雷德蒙德

## 简介

全球最大软件公司之一，旗下三大业务板块：
1. **Productivity & Business Processes**：Microsoft 365、Teams、LinkedIn、Dynamics
2. **Intelligent Cloud**：Azure 云平台、AI 服务、GitHub Copilot
3. **More Personal Computing**：Windows、Xbox、Surface、Bing/Edge

## AI 战略定位

- **Azure OpenAI Service**：独家合作伙伴，将 GPT 系列模型集成入云平台
- **M365 Copilot**：企业 AI 助手，从功能产品向企业软件平台层演进
- **GitHub Copilot**：代码 AI 助手，开发者生产力工具
- CEO Satya Nadella 将 AI 定位为"下一代操作系统"

## 关键指标（FY26E，MS 预测）

| 指标 | 数值 |
|------|------|
| 收入 | $328.6bn |
| 营业利润率 | 46.2% |
| Azure 增速 cc | 30.2% |
| Capex（含资本租赁） | ~$143.9bn |
| M365 Copilot 付费席位 | 1500万（F2Q26） |

## Sources

- 2026-04-20 · MS · [Microsoft 3Q26 前瞻：Azure/Copilot/Capex](../notes/microsoft-3q26-preview-copilot-azure-ms-2026.md)
- 2026-04-15 · MS · [AI 基础设施表外承诺与杠杆风险](../notes/ai-infra-off-balance-sheet-leverage-ms-2026.md)
- 2026-04-14 · Bernstein · [GenAI 全球软件 5 年展望](../notes/genai-global-software-future-bernstein-2026.md)
